...
# CORRECTED: Cleaned list definition to avoid previous SyntaxError
community_columns = [
...
# ADDED: Major Region Mapping Function (From Request 1)
def get_major_region(region_str):
    """Maps the Primary_Region column to the desired Major Region categories (Houston, DFW, etc.)."""
    if pd.isna(region_str) or str(region_str).strip() in ['Not Specified', '(No value)', '']:
        return 'Other/Not Specified'

    # Standardize the string for easier parsing and split into components
    region_str_clean = str(region_str).upper().replace(';', ',').replace(' AND ', ',')
    region_mentions = [r.strip() for r in region_str_clean.split(',') if r.strip()]

    # Priority-based assignment (Houston > DFW > Austin > San Antonio > Central Texas)
    dfw_keywords = ['DFW', 'DALLAS']
    houston_keywords = ['HOUSTON']
    
    if any(k in r for r in region_mentions for k in houston_keywords):
        return 'Houston'
    if any(k in r for r in region_mentions for k in dfw_keywords):
        return 'DFW'
    if 'AUSTIN' in region_mentions:
        return 'Austin'
    if 'SAN ANTONIO' in region_mentions:
        return 'San Antonio'
    if 'CENTRAL TEXAS' in region_mentions:
        return 'Central Texas'

    if 'DALLAS' in region_mentions or 'DFW' in region_mentions:
        return 'DFW'
...
# Determine which Primary Region column to use
base_col = 'Chesmar Primary Regions of Interest'
...
# **START REQUEST 1 APPEND: Add Major_Region to validation_report**
validation_report['Major_Region'] = validation_report['Primary_Regions_of_Interest'].apply(get_major_region)
# **END REQUEST 1 APPEND**
...
# Save final outputs (Absolute paths converted to relative paths)
# NOTE: The following saves are retained to ensure consistency with prior steps, 
# even though the main outputs are saved above.
Out5 = validation_report.to_csv('validation_report.csv', index = None, header=True)
Out5 = merged_df.to_csv('merged_df.csv', index = None, header=True)

# **START REQUEST 2 APPEND: Create and save final_presentation_summary_df**

# Create the final presentation summary DataFrame (static data)
data = {
    'Major Region': ['Houston', 'Dallas/Fort Worth', 'San Antonio', 'Austin', 'Central Texas', 'Other/Not Specified'],
    'Conversions (MQL->SQL)': [4025, 164, 170, 183, 69, 7],
    'Avg. Latency (Days)': [33, 19, 20, 11, 22, 15],
    'Median Latency (Days)': [26, 1, 11, 1, 1, 13],
    'Avg. Quality Score': [70, 59, 54, 63, 55, 72],
    'Avg. Community Overlap (%)': [47, 27, 19, 26, 20, 42]
}

final_presentation_summary_df = pd.DataFrame(data)

# Reorder and sort columns for presentation
final_presentation_summary_df = final_presentation_summary_df[[
    'Major Region', 'Conversions (MQL->SQL)', 'Avg. Latency (Days)',
    'Median Latency (Days)', 'Avg. Quality Score', 'Avg. Community Overlap (%)'
]]

# Sort to maintain the defined order (Houston, DFW, etc.)
order = ['Houston', 'Dallas/Fort Worth', 'San Antonio', 'Austin', 'Central Texas', 'Other/Not Specified']
final_presentation_summary_df['Major Region'] = pd.Categorical(
    final_presentation_summary_df['Major Region'], 
    categories=order, 
    ordered=True
)
final_presentation_summary_df = final_presentation_summary_df.sort_values('Major Region').reset_index(drop=True)

final_presentation_output = 'final_regional_summary_presentation.csv'
final_presentation_summary_df.to_csv(final_presentation_output, index=False)

print("\n" + "="*80)
print("FINAL PRESENTATION SUMMARY (Static Data Request)")
print("="*80)
# FIX APPLIED: Removed 'index=False' from to_markdown call to resolve the TypeError
print(final_presentation_summary_df.to_markdown(numalign="left", stralign="left")) 
print(f"\n✓ Final presentation summary DataFrame saved to: {final_presentation_output}")

# **END REQUEST 2 APPEND**
# **END REQUEST 2 APPEND**